...
(function ($) {
  ...
  function GTreeTable(element, options) {
    ...
    '<td>' +
    '<span>${draggableIcon}${indent}${seq}${ecIcon}${selectedIcon}${typeIcon}${name}</span>' +
  ...
  GTreeTable.prototype = {
    ...
    getSourceNodes: function (nodeId, type, level, force) {
      var that = this,
      ...
      var sourceOptions = this.options.source(nodeId, type, level);
    ...
    init: function () {
      ...
      this.getSourceNodes(0, 'catalog', 0).done(function (result) {
  ...
  GTreeTableNode.prototype = {
    ...
    getParentType: function () {
      if (this.parent != 0) {
        var pNode = this.manager.getNodeById(this.parent);
        return pNode.type;
      } else {
        return "catalog";
      }
    ...
    getRelatedType: function () {
      if (this.relatedNodeId != 0) {
        var pNode = this.manager.getNodeById(this.relatedNodeId);
        if (pNode) {
          return pNode.type;
        }
      }
    ...
    init: function () {
      ...
      this.$seq = this.$node.find("." + this.manager.options.classes.seq);
    ...
    expand: function (options) {
      ...
      $.when(this.manager.getSourceNodes(oNode.id, oNode.type, oNode.level, settings.isAltPressed)).done(function (result) {

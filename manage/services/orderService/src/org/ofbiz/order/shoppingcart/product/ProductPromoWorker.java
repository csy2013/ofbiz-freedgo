...
public class ProductPromoWorker {
    ...
    /**
     * 先检查订单限制useLimitPerOrder->客户使用限制useLimitPerCustomer->促销限制useLimitPerPromotion
     * 通常客户使用限制和促销限制不做限制，模式为空
     * 如果客户只能使用该促销一次则useLimitPerCustomer=1
     */
...
//                        performActions = false;
                    ...
                    //add by changsy 20160412  每个促销规则PromoRule 对应多个PromoRuleCondition,多个PromoRuleAction, 现状所有的PromoRuleCond都满足后才执行所有的Action
                    // 修改成判断当前的PromoRuleCondition如果满足就执行对应的PromAction ,建立cond->Action对应关系，从数据库表层面。
                    if (performActions) {
                        // perform all actions, either apply or unapply
//                        List<GenericValue> productPromoActions = productPromoRule.getRelatedCache("ProductPromoAction", null, UtilMisc.toList("productPromoActionSeqId"));
                        List<GenericValue> productPromoActions = productPromoCond.getRelatedCache("ProductPromoAction", null, UtilMisc.toList("productPromoActionSeqId"));
                        Iterator<GenericValue> productPromoActionIter = UtilMisc.toIterator(productPromoActions);
                        while (productPromoActionIter != null && productPromoActionIter.hasNext()) {
                            GenericValue productPromoAction = productPromoActionIter.next();
                            try {
    ...
    protected static boolean checkCondition(GenericValue productPromoCond, ShoppingCart cart, Delegator delegator, LocalDispatcher dispatcher, Timestamp nowTimestamp) throws GenericEntityException {
        ...
        if ("PPIP_SERVICE".equals(inputParamEnumId)) {
            ...
            while (quantityNeeded.compareTo(BigDecimal.ZERO) > 0 && lineOrderedByBasePriceIter.hasNext()) {
                ...
                GenericValue product = cartItem.getProduct();
                String parentProductId = cartItem.getParentProductId();
//                如果产品没有标价（list price)默认passItemConds true
                boolean passedItemConds = checkConditionsForItem(productPromoCond, cart, cartItem, delegator, dispatcher, nowTimestamp);
                if (passedItemConds && !cartItem.getIsPromo() &&
                        (productIds.contains(cartItem.getProductId()) || (parentProductId != null && productIds.contains(parentProductId))) &&
                        (product == null || !"N".equals(product.getString("includeInPromotions")))) {
        ...
        */
            ...
            // does nothing on order level, only checked on item level, so ignore by always considering passed
            return true;
        } //团购（判断当有效订单数是否大于等于condvalue，小于 othenvalue）
        else if("PPIP_GRPODR_TOTAL".equals(inputParamEnumId)){
            compareBase = Integer.valueOf(-1);
            Integer orderSubTotal = 0;
            if(UtilValidate.isNotEmpty(condValue)){
                Set<String> productIds = ProductPromoWorker.getPromoRuleCondProductIds(productPromoCond, delegator, nowTimestamp);
                if(UtilValidate.isNotEmpty(productIds)){
                    ...
                    try{
                        Map<String,Object> result = dispatcher.runSync("getAvailableGroupOrderQuantity",UtilMisc.toMap("productId",productId,"activityId",groupOrderId,"userLogin",userLogin));
                        orderSubTotal = (Integer) result.get("quantities");
                        if (ServiceUtil.isError(result)) {
                            Debug.logError("Error calling getAvailableGroupOrderQuantity service for the PPIP_GRPODR_TOTAL ProductPromo condition input value: " + ServiceUtil.getErrorMessage(result), module);
                            return false;
                        }
                    } catch (GenericServiceException e) {
                        Debug.logError(e, "Error getting order history sub-total in the getOrderedSummaryInformation service, evaluating condition to false.", module);
            ...
            boolean pass = false;
            compareBase = Integer.valueOf(orderSubTotal.compareTo(Integer.valueOf(condValue)));
            if(compareBase>=0){
                if(UtilValidate.isNotEmpty(otherValue)){
                    compareBase = Integer.valueOf(orderSubTotal.compareTo(Integer.valueOf(otherValue)));
                    if(compareBase<0){
                        pass = true;
                    }
                }else{
        ...
        //秒杀
        else if("SALE_TIME_BTW".equals(inputParamEnumId)){
            boolean pass = false;
            nowTimestamp = UtilDateTime.nowTimestamp();
            try {
                Timestamp startDate = (Timestamp)ObjectType.simpleTypeConvert(condValue, "Timestamp", null, null, cart.getLocale(), true);
                Timestamp endDate = (Timestamp)ObjectType.simpleTypeConvert(otherValue, "Timestamp", null, null, cart.getLocale(), true);
                if(nowTimestamp.compareTo(startDate)>=0 && nowTimestamp.compareTo(endDate)<0){
                    pass = true;
                }
...
//                    BigDecimal quantityUsed = cartItem.addPromoQuantityCandidateUse(quantityDesired, productPromoAction, false);
                    BigDecimal quantityUsed = new BigDecimal(1);
            ...
            while (quantityDesired.compareTo(BigDecimal.ZERO) > 0 && lineOrderedByBasePriceIter.hasNext()) {
                ...
                if (passedItemConds && !cartItem.getIsPromo() &&
                    ...
                    // reduce quantity still needed to qualify for promo (quantityNeeded)
//                    BigDecimal quantityUsed = cartItem.addPromoQuantityCandidateUse(quantityDesired, productPromoAction, false);
//                    quantityUsed = cartItem.getPromoQuantityAvailable();
//                    add by changsy 2016.04.12 促销的复杂性，目前预定始终为1
                    BigDecimal quantityUsed = new BigDecimal(1);
                    quantityDesired = quantityDesired.subtract(quantityUsed);
            ...
            //满足条件执行后 quantityDesired个数量的产品的总价格为：desiredAmount，即这么多产品一起是多少钱
            ...
            BigDecimal quantityDesired = productPromoAction.get("quantity") == null ? BigDecimal.ONE : productPromoAction.getBigDecimal("quantity"); //期望的数量
            BigDecimal desiredAmount = productPromoAction.get("amount") == null ? BigDecimal.ZERO : productPromoAction.getBigDecimal("amount"); //去掉的价格
            ...
            while (quantityDesired.compareTo(BigDecimal.ZERO) > 0 && lineOrderedByBasePriceIter.hasNext()) {
                ...
                if (passedItemConds && !cartItem.getIsPromo() && (productIds.contains(cartItem.getProductId()) || (parentProductId != null && productIds.contains(parentProductId))) &&
                        (product == null || !"N".equals(product.getString("includeInPromotions")))) {
                    //如果cartItem不是促销，(包含产品，或者促销未选择产品并且当前产品可以使用促销
                    // reduce quantity still needed to qualify for promo (quantityNeeded)
//                    add by changsy 2016.04.12 促销的复杂性，目前预定始终为1
                    BigDecimal quantityUsed = new BigDecimal(1);
//                    BigDecimal quantityUsed = cartItem.addPromoQuantityCandidateUse(quantityDesired, productPromoAction, false);
            ...
            if (totalAmount.compareTo(desiredAmount) > 0 && quantityDesired.compareTo(BigDecimal.ZERO) == 0) {
                BigDecimal discountAmountTotal = totalAmount.subtract(desiredAmount).negate(); //获取打折了多少金额(负数）
                distributeDiscountAmount(discountAmountTotal, totalAmount, cartItemsUsed, productPromoAction, delegator); //参数（折扣价格，折扣前价格，使用的cartItems。
                actionResultInfo.ranAction = true;
                actionResultInfo.totalDiscountAmount = discountAmountTotal;
        ...
        //增加按照指定的价格
//        else if ("PROMO_PROD_ASGPC".equals(productPromoActionEnumId)) {
//            BigDecimal quantityDesired = productPromoAction.get("quantity") == null ? BigDecimal.ONE : productPromoAction.getBigDecimal("quantity");
//            // 指定数量默认1
...
//                }
//                BigDecimal subTotal = cart.getSubTotalForPromotions();
//                if (subTotal.compareTo(desiredAmount) > 0) {
//                    BigDecimal amount = subTotal.subtract(desiredAmount);// 获取优惠的金额
//                    amount = amount.negate();
//                    if (amount.compareTo(subTotal) > 0) {
//                        amount = subTotal.negate();
//                    }
//                    doOrderPromoAction(productPromoAction, cart, amount, "amount", delegator);//执行指定的价格
//                    actionResultInfo.ranAction = true;
//                    actionResultInfo.totalDiscountAmount = amount;
//                }
//            }
        //增加按照指定的价格
            ...
            while (quantityDesired.compareTo(BigDecimal.ZERO) > 0 && lineOrderedByBasePriceIter.hasNext()) {
                ...
                if (passedItemConds && !cartItem.getIsPromo() && (productIds.contains(cartItem.getProductId()) || (parentProductId != null && productIds.contains(parentProductId))) &&
                    ...
                    //如果cartItem不是促销，(包含产品，或者促销未选择产品并且当前产品可以使用促销
                    // reduce quantity still needed to qualify for promo (quantityNeeded)
//                    cartItem可以用在团购产品数量,此团购不考虑promoQuantityUsed
                    BigDecimal quantityUsed = cartItem.getQuantity();
//                    cartItem.getPromoQuantityAvailable();//可用数量为cartItem数量减去条件值里面数量
                    if (quantityUsed.compareTo(BigDecimal.ZERO) > 0) {
                        //计算所有适用产品的折扣价总和，及折扣前价格totalAmount总和
                        amount = amount.add(quantityUsed.multiply(desiredAmount));
                        totalAmount = totalAmount.add(quantityUsed.multiply(cartItem.getBasePrice()).multiply(cartItem.getRentalAdjustment()));
                        cartItemsUsed.add(cartItem);
                    }
                }
            ...
            if (totalAmount.compareTo(desiredAmount) > 0) {
                BigDecimal discountAmountTotal = totalAmount.subtract(amount).negate();

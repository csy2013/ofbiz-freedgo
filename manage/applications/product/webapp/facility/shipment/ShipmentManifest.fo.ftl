                ...
                <fo:table-header>
                    <fo:table-row font-weight="bold">
                        ...
                        <fo:table-cell padding="2pt" background-color="#D4D0C8">
                            ...
                            <fo:block text-align="center">${uiLabelMap.ProductCarrier}
                                : <#if carrierParty.description?has_content>${carrierParty.description}<#else>${carrierParty.partyId}</#if></fo:block>
                        </fo:table-cell>
                        <fo:table-cell padding="2pt" background-color="#D4D0C8">
                            <#assign shipmentMethodType = shipmentRouteSegment.getRelatedOne("ShipmentMethodType")?if_exists>
                            <fo:block text-align="right">${uiLabelMap.ProductShipmentMethod}
                                : <#if shipmentMethodType?has_content>${shipmentMethodType.description}<#else>${uiLabelMap.CommonNA}</#if></fo:block>
                        </fo:table-cell>
                    </fo:table-row>
                </fo:table-header>
                <fo:table-body>
                        ...
                        <fo:table-cell padding="2pt">
                            <fo:block>
                                <#if originPostalAddress?has_content>
                                    ...
                                    <fo:block>
                                        <#if originPostalAddress.stateProvinceGeoId?has_content>${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(originPostalAddress.stateProvinceGeoId,delegator)}</#if>
                                    ${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(originPostalAddress.city?if_exists,delegator)}
                                    ${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(originPostalAddress.countyGeoId?if_exists,delegator)}
                                    </fo:block>
                                    <fo:block>${originPostalAddress.address1?if_exists}</fo:block>
                                    <fo:block>${originPostalAddress.address2?if_exists}</fo:block>
                                    <fo:block>${originPostalAddress.postalCode?if_exists}</fo:block>
                        ...
                        <fo:table-cell padding="2pt">
                            <fo:block text-align="center">
                                <#if destinationPostalAddress?has_content>
                                    ...
                                    <fo:block>
                                        <#if destinationPostalAddress.stateProvinceGeoId?has_content>${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(destinationPostalAddress.stateProvinceGeoId,delegator)}</#if>
                                    ${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(destinationPostalAddress.city?if_exists,delegator)}
                                     ${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(destinationPostalAddress.countyGeoId?if_exists,delegator)}
                                    ...
                                    <fo:block>${destinationPostalAddress.postalCode?if_exists}</fo:block>

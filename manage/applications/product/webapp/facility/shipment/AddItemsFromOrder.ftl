<#if shipment?exists>
    <@htmlScreenTemplate.renderScreenletBegin id="" title="${uiLabelMap.PageTitleAddItemsFromOrder}"/>
<form name="additemsfromorder" action="<@ofbizUrl>AddItemsFromOrder</@ofbizUrl>" class="form-inline">
    <input type="hidden" name="shipmentId" value="${shipmentId}"/>
    <div>
        <span class="label1">${uiLabelMap.ProductOrderId}</span>
        <span>
        ...
        <span class="label1">${uiLabelMap.ProductOrderShipGroupId}</span>
    ...
    <@htmlScreenTemplate.renderScreenletBegin id="ProductAddItemsShipment" title="${uiLabelMap.ProductAddItemsShipment}: [${shipmentId?if_exists}]; ${uiLabelMap.ProductFromAnOrder}: [${orderId?if_exists}], ${uiLabelMap.ProductOrderShipGroupId}:
                [${shipGroupSeqId?if_exists}]"/>
    ...
    <#if orderHeader?exists>
        ...
        <h3 style="color: red;">${uiLabelMap.ProductWarningOrderType} ${(orderType.get("description",locale))?default(orderHeader.orderTypeId?if_exists)}
        ...
        <h3>${uiLabelMap.ProductOriginFacilityIs}: <#if originFacility?exists>${originFacility.facilityName?if_exists} [${originFacility.facilityId}]<#else><span
                style="color: red;">${uiLabelMap.ProductNotSet}</span></#if></h3>
        <#elseif shipment.shipmentTypeId?if_exists == "PURCHASE_SHIPMENT">
        <h3>${uiLabelMap.ProductDestinationFacilityIs}: <#if destinationFacility?exists>${destinationFacility.facilityName?if_exists} [${destinationFacility.facilityId}]<#else><span
                style="color: red;">${uiLabelMap.ProductNotSet}</span></#if></h3>
        </#if>
        ...
        <#else>
        <h3 style="color: red;">${uiLabelMap.ProductWarningOrderStatus} ${(orderHeaderStatus.get("description",locale))?default(orderHeader.statusId?if_exists)}
        ...
        <table class="table table-bordered table-responsive">
            <tr class="header-row">
                ...
                <td align="right">
                    ...
                    <div>${uiLabelMap.CommonAll}<input type="checkbox" name="selectAll" value="${uiLabelMap.CommonY}"
                                                       onclick="toggleAll(this, 'selectAllForm');highlightAllRows(this, 'orderItemData_tableRow_', 'selectAllForm');"/></div>
                </td>
            </tr>
            ...
            <#list orderItemDatas?if_exists as orderItemData>
                ...
                <tr id="orderItemData_tableRow_${rowCount}" valign="middle"<#if alt_row> class="alternate-row"</#if>>
                    <td>
                        <div>${orderItemAndShipGroupAssoc.orderId} / ${orderItemAndShipGroupAssoc.shipGroupSeqId} / ${orderItemAndShipGroupAssoc.orderItemSeqId}</div>
                    </td>
                    <td>
                        <div>${(product.internalName)?if_exists} [${orderItemAndShipGroupAssoc.productId?default("N/A")}]</div>
                    </td>
                    <td>
                        <#if itemIssuances?has_content>
                            <#list itemIssuances as itemIssuance>
                                <div><b>[${itemIssuance.quantity?if_exists}]</b>${itemIssuance.shipmentId?if_exists}:${itemIssuance.shipmentItemSeqId?if_exists} ${uiLabelMap.CommonOn}
                                    [${(itemIssuance.issuedDateTime.toString())?if_exists}] ${uiLabelMap.CommonBy} [${(itemIssuance.issuedByUserLoginId)?if_exists}]
                                </div>
                            </#list>
                        <#else>
                            <div>&nbsp;</div>
                    ...
                    <td>
                        <div>
                            <#if isSalesOrder>
                                ...
                                <b>
                                        <#if (totalQuantityIssuedAndReserved > orderItemAndShipGroupAssoc.quantity)>
                                            &gt;<#else><#if (totalQuantityIssuedAndReserved < orderItemAndShipGroupAssoc.quantity)>&lt;<#else>=</#if></#if>
                            ...
                            ${totalQuantityIssued}
                                <b>
                                        <#if (totalQuantityIssued > orderItemAndShipGroupAssoc.quantity)>&gt;<#else><#if (totalQuantityIssued < orderItemAndShipGroupAssoc.quantity)>
                                            &lt;<#else>=</#if></#if>
                                ${orderItemAndShipGroupAssoc.quantity}
                                    </b>
                            </span>
                    ...
                    <#if isSalesOrder>
                        ...
                        <td align="right">
                            <input type="checkbox" name="_rowSubmit_o_${rowCount}" value="Y"
                                   onclick="checkToggle(this, 'selectAllForm');highlightRow(this,'orderItemData_tableRow_${rowCount}');"/>
                        </td>
                    <#else>
                            ...
                            <td>
                                ...
                                ${orderItemShipGrpInvRes.inventoryItemId}
                                    <#if inventoryItem.facilityId?has_content>
                                        <span<#if originFacility?exists && originFacility.facilityId != inventoryItem.facilityId>
                                                style="color: red;"</#if>>[${(inventoryItemFacility.facilityName)?default(inventoryItem.facilityId)}]</span>
                                    <#else>

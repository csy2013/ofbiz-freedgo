<@htmlScreenTemplate.renderScreenletBegin id="" title="${uiLabelMap.ProductReceiveReturn} ${uiLabelMap.CommonInto} ${uiLabelMap.CommonId}:${facility.facilityId?if_exists}]"/>
    <#--<a href="<@ofbizUrl>EditFacility</@ofbizUrl>" class="buttontext">${uiLabelMap.ProductNewFacility}</a>-->
        <#-- Receiving Results -->
        <#if receivedItems?has_content>
          <h4>${uiLabelMap.ProductReceiptForReturn} ${uiLabelMap.CommonNbr}<a href="/ordermgr/control/returnMain?returnId=${returnHeader.returnId}${externalKeyParam?if_exists}" class="buttontext">${returnHeader.returnId}</a></h4>
          <#if "RETURN_RECEIVED" == returnHeader.getString("statusId")>
            <h4>${uiLabelMap.ProductReturnCompletelyReceived}</h4>
          </#if>
          <br />
        ...
        <#if returnHeader?has_content>
                ...
                <#list returnItems as returnItem>
                  ...
                  <tr>
                    <td>
                        ...
                        <tr>
                           <td width='10%'>
                              <select name="inventoryItemTypeId_o_${rowCount}" size="1" id="inventoryItemTypeId_o_${rowCount}" onchange="setInventoryItemStatus(this,${rowCount});">
          ...
          <#-- Initial Screen -->
        <#else>
          <form name="selectAllForm" method="post" action="<@ofbizUrl>ReceiveReturn</@ofbizUrl>" class="form-inline">
            <input type="hidden" name="facilityId" value="${requestParameters.facilityId?if_exists}" />
            <input type="hidden" name="initialSelected" value="Y" />
              <div class="form-group">
                <div class="input-group"><div class="input-group-addon"><span> ${uiLabelMap.ProductReturnNumber}</span></div>
                  <input type="text" name="returnId" size="20" maxlength="20" value="${requestParameters.returnId?if_exists}" class="form-control"/>
                </div>

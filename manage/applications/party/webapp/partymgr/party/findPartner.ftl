...
<#if !requestParameters.ajaxUpdateEvent?exists>
    <@htmlScreenTemplate.renderScreenletBegin id="findPartyScreen" title="${uiLabelMap.PageTitleFindPartner}"/>
<form method="post" name="lookupparty" action="<@ofbizUrl>findparty</@ofbizUrl>" class="form-inline" id="lookupparty">
    ...
    <#--<div class= "input-group m-b-10 m-r-5"><span class="input-group-addon">${uiLabelMap.PartyType}</span>
        ...
        <input type="hidden" name="partyTypeId" value="PARTNER"/>
...
<#if partyList?exists>
    ...
    <div class="button-bar"><a href="<@ofbizUrl>NewPartner</@ofbizUrl>" class="btn btn-primary btn-sm">${uiLabelMap.CommonCreateNew}</a></div>
    </#if>
<div class="table-responsive" id="search-results">
          ...
          <#if partyRelateCom?has_content>
              ...
              ${mainRole.description?if_exists}
                    ...
                    <#-- <#if security.hasEntityPermission("ORDERMGR", "_CREATE", session)>
                        ...
                        confirmTitle="删除合作伙伴" confirmMessage="确定删除该合作伙伴" targetParameterIter="partyId:'${partyRow.partyId}'"
    ...
    <#else>
        <div id="findPartyResults_2">
            <h3>${uiLabelMap.PartnerNoPartiesFound}</h3>
        </div>
    </#if>
</div>

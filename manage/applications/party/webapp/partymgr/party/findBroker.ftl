...
<#if !requestParameters.ajaxUpdateEvent?exists>
    <@htmlScreenTemplate.renderScreenletBegin id="findPartyScreen" title="${uiLabelMap.PageTitleFindBroker}"/>
<form method="post" name="lookupparty" action="<@ofbizUrl>findparty</@ofbizUrl>" class="form-inline" id="lookupparty">
    ...
    <#--<div class= "input-group m-b-10 m-r-5"><span class="input-group-addon">${uiLabelMap.PartyType}</span>
        ...
        <input type="hidden" name="partyTypeId" value="BROKER"/>
...
<#if partyList?exists>
    ...
    <div class="button-bar"><a href="<@ofbizUrl>NewBroker</@ofbizUrl>" class="btn btn-primary btn-sm">${uiLabelMap.CommonCreateNew}</a></div>
    </#if>
<div class="table-responsive" id="search-results">
    ...
    <#else>
        <div id="findPartyResults_2">
            <h3>${uiLabelMap.BrokerNoPartiesFound}</h3>
        </div>
    </#if>
</div>

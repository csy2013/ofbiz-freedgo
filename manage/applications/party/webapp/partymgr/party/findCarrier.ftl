...
<#if !requestParameters.ajaxUpdateEvent?exists>
    <@htmlScreenTemplate.renderScreenletBegin id="findPartyScreen" title="${uiLabelMap.PageTitleFindCarrier}"/>
<form method="post" name="lookupparty" action="<@ofbizUrl>findparty</@ofbizUrl>" class="form-inline" id="lookupparty">
    ...
    <#--<div class= "input-group m-b-10 m-r-5"><span class="input-group-addon">${uiLabelMap.PartyType}</span>
        ...
        <input type="hidden" name="partyTypeId" value="CARRIER"/>
...
<#if partyList?exists>
    ...
    <div class="button-bar"><a href="<@ofbizUrl>NewCarrier</@ofbizUrl>" class="btn btn-primary btn-sm">${uiLabelMap.CommonCreateNew}</a></div>
    </#if>
<div class="table-responsive" id="search-results">
        ...
        <table class="table table-bordered table-striped" cellspacing="0">
          ...
          <#if partyRelateCom?has_content>
              ...
              ${mainRole.description?if_exists}
                ...
                confirmTitle="删除承运商" confirmMessage="确定删除该承运商" targetParameterIter="partyId:'${partyRow.partyId}'"
                confirmUrl="deleteParty" description="删除"/>

                        <#--<a class="btn btn-primary btn-sm" href="<@ofbizUrl>viewprofile?partyId=${parxtyRow.partyId}</@ofbizUrl>">${uiLabelMap.CommonDetails}</a>-->
    ...
    <#else>
        <div id="findPartyResults_2">
            <h3>${uiLabelMap.CarrierNoPartiesFound}</h3>
        </div>
    </#if>
</div>

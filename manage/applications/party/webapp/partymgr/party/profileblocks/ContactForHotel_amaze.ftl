...
<div class="am-cf am-padding-xs">
    <div id="partyUserLogins" class="am-panel am-panel-default">
        <div class="am-panel-hd am-cf">
            <div class="am-u-lg-8">${uiLabelMap.PartyContactInformation}</div>
        <#if security.hasEntityPermission("PARTYMGR", "_CREATE", session) || userLogin.partyId == partyId>
           <div  class="am-u-lg-2"><a href="<@ofbizUrl>editcontactmechforhotel?partyId=${partyId}</@ofbizUrl>">${uiLabelMap.CommonCreateNew}</a></div>
        </#if>
      </div>
      <br class="clear" />
            ...
            <tr>
              ...
              <td>
                ...
                <#elseif "EMAIL_ADDRESS" = contactMech.contactMechTypeId>
                  <div>
                    ...
                    <form method="post" action="<@ofbizUrl>NewDraftCommunicationEvent</@ofbizUrl>" onsubmit="submitFormDisableSubmits(this)" name="createEmail${contactMech.infoString?replace("&#64;","")?replace(".","")}">
                ...
                <div>(${uiLabelMap.CommonUpdated}:&nbsp;${partyContactMech.fromDate?string("yyyy-MM-dd HH:mm:ss")!})</div>
              ...
              <td class="button-col">
                <#if security.hasEntityPermission("PARTYMGR", "_UPDATE", session) || userLogin.partyId == partyId>
                  <a class="am-btn am-btn-primary am-btn-sm" href="<@ofbizUrl>editcontactmechforhotel?partyId=${partyId}&amp;contactMechId=${contactMech.contactMechId}</@ofbizUrl>">${uiLabelMap.CommonUpdate}</a>
                </#if><br/><br/>
                <#if security.hasEntityPermission("PARTYMGR", "_DELETE", session) || userLogin.partyId == partyId>
                  <form name="partyDeleteContact" method="post" action="<@ofbizUrl>deleteContactMechforhotel</@ofbizUrl>" onsubmit="submitFormDisableSubmits(this)">
                    <input name="partyId" value="${partyId}" type="hidden"/>
                    <input name="contactMechId" value="${contactMech.contactMechId}" type="hidden"/>
                    <input type="submit" class="am-btn am-btn-primary am-btn-sm" value="${uiLabelMap.CommonExpire}"/>
                  </form>
                </#if>
              </td>
            </tr>

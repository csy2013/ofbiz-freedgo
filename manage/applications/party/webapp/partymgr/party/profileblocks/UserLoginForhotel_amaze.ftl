...
<div id="partyUserLogins" class="am-panel am-panel-default">
        ...
        <#if security.hasEntityPermission("PARTYMGR", "_CREATE", session)>
            <div  class="am-u-lg-2"> <a href="<@ofbizUrl>ProfileCreateNewLoginForHotel?partyId=${party.partyId}</@ofbizUrl>">${uiLabelMap.CommonCreateNew}</a></div>
        </#if>
        </div>
      <br class="clear" />
        ...
        <table  cellspacing="0" style="border-collapse: separate;border-spacing: 0;margin-bottom: 1.6rem;width: 100%;">
          <#list userLogins as userUserLogin>
            <tr>
              ...
              <td class="button-col">
                <#if security.hasEntityPermission("PARTYMGR", "_CREATE", session)>
                  <a class="am-btn am-btn-primary am-btn-sm" href="<@ofbizUrl>ProfileEditUserLoginForHotel?partyId=${party.partyId}&amp;userLoginId=${userUserLogin.userLoginId}</@ofbizUrl>">${uiLabelMap.CommonEdit}</a>
                </#if>
                <#if security.hasEntityPermission("SECURITY", "_VIEW", session)>
                  <a class="am-btn am-btn-primary am-btn-sm" href="<@ofbizUrl>ProfileEditUserLoginSecurityGroupsForHotel?partyId=${party.partyId}&amp;userLoginId=${userUserLogin.userLoginId}</@ofbizUrl>">${uiLabelMap.SecurityGroups}</a>
                </#if>
              </td>
            </tr>
          </#list>

...
<%@ page import="org.ofbiz.base.config.ResourceHandler" %>
<%@ page import="org.ofbiz.base.util.UtilFormatOut, org.ofbiz.base.util.UtilMisc" %>
...
<FORM method="post" action='<ofbiz:url>/view/EditEntity?entityName=<%=entityName%>&event=updateEntity</ofbiz:url>' style='margin: 0;'>
  ...
  <%boolean isFile = entityResourceHandler == null || entityResourceHandler.isFileResource();%>
  <TR>
    <TD>Resource Loader</TD>
    <TD><INPUT type="text" class='inputBox' size='20' name='loaderName' value='<%=entityResourceHandler == null ? "" : UtilFormatOut.checkNull(entityResourceHandler.getLoaderName())%>'<%if(!isFile){%> disabled<%}%>></TD>
  </TR>
  <TR>
    <TD>Location</TD>
    <TD><INPUT type="text" class='inputBox' size='60' name='location' value='<%=entityResourceHandler == null ? "" : UtilFormatOut.checkNull(entityResourceHandler.getLocation())%>'<%if(!isFile){%> disabled<%}%>></TD>
  </TR>
  </TABLE>

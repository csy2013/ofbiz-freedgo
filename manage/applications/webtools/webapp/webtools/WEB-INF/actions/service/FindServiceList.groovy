...
selDisp = parameters.selDisp ?: "common"
...
dispArrList.addAll(curDispatchContext.modelServiceComponentNames)
...
if(selectedService){
    context.selectedServiceMap = getServiceInfo(selectedService)
}
if (!selectedService) {
    //get constraints if any
    constraint = parameters.constraint

    serviceNames = curDispatchContext.getServiceNamesByComponentName(selDisp)
    ...
    serviceNames.each { serviceName ->
        ...
        servicesList.add(getServiceInfo(serviceName))
        servicesFoundCount++
    }
...
Map getServiceInfo(String selectedService){
    curServiceMap = [:]
    if (selectedService) {

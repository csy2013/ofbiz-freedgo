        ...
        <div align="right">
          <#if grandTotal?exists>
            <#if glReconciliations?has_content>
              ...
              <input id="submitButton" type="submit" onclick="document.selectAllForm.submit();" value="${uiLabelMap.AccountingAssignToReconciliation}" disabled="disabled" />
            <#else>
          ...
          <#else>
            <input id="submitButton" type="submit" onclick="document.selectAllForm.submit();" value="${uiLabelMap.AccountingReconcile}" disabled="disabled" />
          </#if>
        </div>
      </#if>
      <table class="basic-table hover-bar" cellspacing="0">
        ...
        <tr class="header-row-2">
          ...
          <#if ((glReconciliationId?has_content && glReconciliationId == "_NA_") && (glReconciliations?has_content && finAccountTransList?has_content)) || !grandTotal?exists>
            <th>${uiLabelMap.CommonSelectAll} <input name="selectAll" type="checkbox" value="N" id="checkAllTransactions" onclick="togglefinAccountTransId(this);"/></th>
          </#if>
        </tr>
        <#-- Header Ends-->
        ...
        <#list finAccountTransList as finAccountTrans>
          ...
          <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
            ...
            <#if ((glReconciliationId?has_content && glReconciliationId == "_NA_") && (glReconciliations?has_content && finAccountTransList?has_content)) || !grandTotal?exists>
              <#if finAccountTrans.statusId == "FINACT_TRNS_CREATED">
                <td><input id="finAccountTransId_${finAccountTrans_index}" name="_rowSubmit_o_${finAccountTrans_index}" type="checkbox" value="Y" onclick="getFinAccountTransRunningTotalAndBalances();"/></td>
              </#if>
            </#if>
          </tr>

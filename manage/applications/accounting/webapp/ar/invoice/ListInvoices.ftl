...
<#if invoiceList?has_content && (parameters.noConditionFind)?if_exists == 'Y'>
  ...
  <form name="listInvoices" id="listInvoices"  method="post" action="">
    <div align="right">
      <select name="serviceName" id="serviceName" onchange="setServiceName(this);">
      ...
      <input id="submitButton" type="button"  onclick="jQuery('#listInvoices').submit();" value="${uiLabelMap.CommonRun}" disabled="disabled" />
    ...
    <table class="basic-table hover-bar jDatatable" cellspacing="0">
      <thead>
        <tr class="header-row-2">
          <th>${uiLabelMap.FormFieldTitle_invoiceId}</th>
          <th>${uiLabelMap.FormFieldTitle_invoiceTypeId}</th>
          ...
          <th align="right">${uiLabelMap.CommonSelectAll} <input type="checkbox" id="checkAllInvoices" name="checkAllInvoices" onchange="toggleInvoiceId(this);"/></th>
        </tr>
      </thead>
      <tbody>
        ...
        <#list invoiceList as invoice>
          ...
          <#assign invoicePaymentInfo = invoicePaymentInfoList.get("invoicePaymentInfoList").get(0)?if_exists>
            <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
              ...
              <td align="right"><input type="checkbox" id="invoiceId_${invoice_index}" name="invoiceIds" value="${invoice.invoiceId}" onclick="getInvoiceRunningTotal();"/></td>
            </tr>

...
<#if invoices?has_content >
  <form name="listSalesInvoices" id="listSalesInvoices" method="post">
    ...
    <div align="right">
      <select name="serviceName" id="serviceName" onchange="setServiceName(this);" class="form-control">
      ...
      <input id="submitButton" type="button" onclick="runAction();" value="${uiLabelMap.CommonRun}" disabled="disabled" />
    </div>
    <table class="basic-table hover-bar" cellspacing="0">
      <#-- Header Begins -->
      <tr class="header-row-2">
        <td width="9%"><input type="checkbox" id="checkAllInvoices" name="checkAllInvoices" onchange="toggleInvoiceId(this);"/> ${uiLabelMap.CommonSelectAll}</td>
      ...
      <#list invoices as invoice>
        ...
        <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
          <td><input type="checkbox" id="invoiceId_${invoice_index}" name="invoiceIds" value="${invoice.invoiceId}" onclick="enableSubmitButton();"/></td>

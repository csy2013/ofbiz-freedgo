    ...
    <#if contactMech.contactMechTypeId == "POSTAL_ADDRESS">
        ...
        <fo:block text-indent="0.2in">
            <#if postalAddress?has_content>
                <fo:block>
                    <#assign stateGeo = (delegator.findOne("Geo", {"geoId", postalAddress.stateProvinceGeoId?if_exists}, false))?if_exists />
                ${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(postalAddress.city,delegator)}
                    <#if stateGeo?has_content>, ${stateGeo.geoName?if_exists}</#if> ${postalAddress.postalCode?if_exists}
                </fo:block>

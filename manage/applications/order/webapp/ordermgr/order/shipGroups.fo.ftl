    ...
    <fo:table-body>
      <fo:table-row>
        <fo:table-cell number-rows-spanned="4">
          ...
          <fo:block>
            ${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(address.city?if_exists,delegator)}<#if address.stateProvinceGeoId?has_content>, ${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(address.stateProvinceGeoId,delegator)}</#if>
            ${address.postalCode?if_exists} <#--${Static["org.ofbiz.common.geo.GeoWorker"].getGeoNameById(address.countryGeoId?if_exists,delegator)}-->
          </fo:block>

...
<tr>
    <td colspan="2">&nbsp;</td>
    <td colspan="3" align="right"><span>${returnAdjustment.getRelatedOne("ReturnAdjustmentType").get("description",locale)?default("N/A")}</span>
...
<#if returnHeader?has_content>
    <#if returnHeader.destinationFacilityId?has_content && returnHeader.statusId == "RETURN_ACCEPTED" && returnHeader.returnHeaderTypeId?starts_with("CUSTOMER_")>
        ...
        <div class="btn-group">
            <#list returnShipmentIds as returnShipmentId>
                <a href="/facility/control/ViewShipment?shipmentId=${returnShipmentId.shipmentId}${externalKeyParam}"
                   class="btn btn-primary btn-sm">${uiLabelMap.ProductShipmentId} ${returnShipmentId.shipmentId}</a>
                <a href="/facility/control/ReceiveReturn?facilityId=${returnHeader.destinationFacilityId}&amp;returnId=${returnHeader.returnId?if_exists}&amp;shipmentId=${returnShipmentId.shipmentId}${externalKeyParam}"
                   class="btn btn-primary btn-sm">${uiLabelMap.OrderReceiveReturn}</a>
            </#list>
        </div>
        <br/>
        <br/>
    ...
    <div class="btn-group">
        ...
        <#else>
            <a href="/facility/control/EditShipment?primaryReturnId=${returnHeader.returnId}&amp;partyIdTo=${toPartyId}&amp;statusId=SHIPMENT_INPUT&amp;shipmentTypeId=PURCHASE_RETURN"
               class="btn btn-primary btn-sm">${uiLabelMap.OrderCreateReturnShipment}</a>
        </#if>
    </div>
    <br/>
    <br/>
...
<form method="post" action="<@ofbizUrl>updateReturnItems</@ofbizUrl>" class="form-horizontal">
    ...
    <div class="table-responsive">
        <table cellspacing="0" class="table table-bordered">
            ...
            <tr>
                <td colspan="10"><h3>${uiLabelMap.OrderOrderReturn} #${returnId}</h3></td>
            </tr>
        ...
        <#-- information about orders and amount refunded/credited on past returns -->
                        ...
                        <td>
                            ...
                            <#if item.receivedQuantity?exists>
                                ...
                                <#list shipmentReceipts?if_exists as shipmentReceipt>
                                    <br/>${uiLabelMap.OrderQty}: ${shipmentReceipt.quantityAccepted}, ${shipmentReceipt.datetimeReceived}, <a
                                        href="/facility/control/EditInventoryItem?inventoryItemId=${shipmentReceipt.inventoryItemId}"
                                ...
                                <#else>
                                    <select name="expectedItemStatus_o_${rowCount}">
                                        ...
                                        <#list itemStatus as returnItemStatus>
                                            <option value="${returnItemStatus.statusId}">${returnItemStatus.get("description")?if_exists}</option>
                                        </#list>
                                    </select>
                                </#if>
                            </div>
                                ...
                                <#if returnHeader.statusId == "RETURN_COMPLETED" || returnHeader.statusId == "SUP_RETURN_COMPLETED">
                                    ...
                                    <#if itemResp?has_content>
                                        <#if itemResp.paymentId?has_content>
                                            <div>${uiLabelMap.AccountingPayment} ${uiLabelMap.CommonNbr}<a
                                                    href="/accounting/control/paymentOverview?paymentId=${itemResp.paymentId}${externalKeyParam}" class="buttontext">${itemResp.paymentId}</a>
                                            </div>
                                        <#elseif itemResp.replacementOrderId?has_content>
                                            <div>${uiLabelMap.OrderOrder} ${uiLabelMap.CommonNbr}<a href="<@ofbizUrl>orderview?orderId=${itemResp.replacementOrderId}</@ofbizUrl>"
                                                                                                    class="buttontext">${itemResp.replacementOrderId}</a></div>
                                        <#elseif itemResp.billingAccountId?has_content>
                                            <div>${uiLabelMap.AccountingAccountId} ${uiLabelMap.CommonNbr}<a
                                                    href="/accounting/control/EditBillingAccount?billingAccountId=${itemResp.billingAccountId}${externalKeyParam}"
                                                    class="buttontext">${itemResp.billingAccountId}</a></div>
                                        </#if>
                                    <#else>
            ...
            <#else>
                <tr>
                    <td colspan="9">
                        <div>${uiLabelMap.OrderNoReturnItemsFound}</div>
                    </td>
                </tr>
            </#if>
    ...
    <form name="returnItems" method="post" action="<@ofbizUrl>returnItems</@ofbizUrl>">
        ...
        <table border='0' cellpadding='2' cellspacing='0'>
            <tr>
                <td colspan="4"><h3>${uiLabelMap.OrderReturnItems}</h3></td>
            </tr>
            <#if partyOrders?has_content>
                <tr>
                    ...
                    <td>
                        <div class="tooltip">${uiLabelMap.OrderReturnLoadItems}</div>
                    </td>
                </tr>
            <#else>
                <tr>
                    <td colspan="4" nowrap="nowrap">
                        <div>${uiLabelMap.OrderNoOrderFoundForParty}: <a href="${customerDetailLink}${partyId?default('_NA_')}" class="buttontext">${partyId?default('[null]')}</a></div>
                    </td>
                </tr>
                <tr>
                    <td width='25%' align='right' nowrap="nowrap">
                        <div>${uiLabelMap.OrderOrderId}</div>
                    </td>
                    <td>&nbsp;</td>
                    <td width='25%'>

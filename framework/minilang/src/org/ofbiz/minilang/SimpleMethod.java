...
public final class SimpleMethod extends MiniLangElement {
    ...
    public String exec(MethodContext methodContext) throws MiniLangException {
        ...
        if (methodContext.getMethodType() == MethodContext.EVENT) {
            ...
            String tempErrorMsg = methodContext.getEnv(eventErrorMessageName);
            ...
            String eventMsg = methodContext.getEnv(eventEventMessageName);
            ...
            response = methodContext.getEnv(eventResponseCodeName);
            ...
            boolean forceError = false;
            String tempErrorMsg = methodContext.getEnv(serviceErrorMessageName);
            if (errorMsg.length() > 0 || UtilValidate.isNotEmpty(tempErrorMsg)) {
                errorMsg += tempErrorMsg;
            ...
            String successMsg = methodContext.getEnv(serviceSuccessMessageName);
            ...
            response = methodContext.getEnv(serviceResponseMessageName);
            if (UtilValidate.isEmpty(response)) {
                if (forceError) {
                    // override response code, always use error code

...
public class MacroScreenRenderer implements ScreenStringRenderer {
    ...
    public void executeMacro(Appendable writer, String macroName, Map<String, Object> parameters) throws IOException {
    ...
    public void renderScreenletBegin(Appendable writer, Map<String, Object> context, boolean collapsed, ModelScreenWidget.Screenlet screenlet) throws IOException {
        ...
        String style = screenlet.getStyle(context);
     ...
     */
    public void renderConfirmModal(Appendable writer, Map<String, Object> context, ModelScreenWidget.ConfirmModal confirmModal) throws IOException {
        Boolean ajaxEnabled = confirmModal.isAjaxEnabled();

        String confirmUrl = confirmModal.getConfirmUrl(context);
        String returnUrl = confirmModal.getReturnUrl(context);
        HttpServletResponse response = (HttpServletResponse) context.get("response");
        HttpServletRequest request = (HttpServletRequest) context.get("request");
        String className = "";
        String alert = "false";
        String confirmationMessage=confirmModal.getConfirmationMsg(context);
        String confirmationTitle = confirmModal.getConfirmTitle(context);
        // add lookup pop-up button
        String description = confirmModal.getDescription(context);
        String targetType = confirmModal.getTargetType();
        ...
        String confirmationPresentation = confirmModal.getConfirmPresentation();
        if(UtilValidate.isEmpty(confirmationPresentation)){
            confirmationPresentation = "";
        }

        String confirmationHeight = confirmModal.getHeight();
        if(UtilValidate.isEmpty(confirmationHeight)){
            confirmationHeight = "";
        }

        String width = confirmModal.getWidth();
        if(UtilValidate.isEmpty(width)){
            width = "";
        }

        String position = confirmModal.getPosition();
        if(UtilValidate.isEmpty(position)){
            position = "";
        }

        String fadeBackground = confirmModal.getFadeBackground();
        if (UtilValidate.isEmpty(fadeBackground)){
            fadeBackground = "false";
        }
        ...
        String name = confirmModal.getName(context);
        String buttonStyle = confirmModal.getButtonStyle();
        String buttonSpanStyle = confirmModal.getButtonSpanStyle();
        String id = confirmModal.getId(context);
        String parameterMapString = confirmModal.getParameterMapString(context);
        String returnParameterMapString =  confirmModal.getReturnParameters(context);
        String buttonType =confirmModal.getButtonType();
        StringWriter sr = new StringWriter();

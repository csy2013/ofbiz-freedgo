    ...
    public void initForm(Element formElement) {
        ...
        if (parentForm.length() > 0) {
            ...
            if (parent != null) {
                ...
                this.onAjaxUpdateAreas = parent.onAjaxUpdateAreas;
...
//                        lastFieldGroup.renderEndString(writer, context, formStringRenderer);
                ...
                //if form field is submit or reset then if isTabCss do 默认最后一个是submit button ，在之前先tab close bug(是否存在两个button，或者没有)?
            ...
            formStringRenderer.renderFormatFieldRowWidgetCellOpen(writer, context, currentFormField, positions, positionSpan, nextPositionInRow, cmaxPosition);

            // render widget
            currentFormField.renderFieldString(writer, context, formStringRenderer);

            // render widget formatting close renderFormatFieldRowWidgetCellClose

 ...
 */
public class ColoradminHtmlMenuRenderer extends HtmlMenuRenderer {

    HttpServletRequest request;
    HttpServletResponse response;

    public ColoradminHtmlMenuRenderer(HttpServletRequest request, HttpServletResponse response) {
    ...
    public void renderMenuOpen(Appendable writer, Map<String, Object> context, ModelMenu modelMenu) throws IOException {
        ...
        // 当菜单没有定义Id，说明是下级菜单，则使用boots的menu div：<nav data-am-widget="menu" class="am-menu  am-menu-default"> ul:am-menu-nav am-avg-sm-3
        ...
        if (UtilValidate.isNotEmpty(menuId)) {
            ...
            String menu_div = UtilProperties.getPropertyValue("coloradmin.properties", "admin.appbar.div");
            ...
            appendWhitespace(writer);
             ...
             if(menuContainerStyle.indexOf("tab-bar")!=-1) {
                 String secondBarNav = UtilProperties.getPropertyValue("coloradmin.properties", "admin.secondbar.nav");
                 writer.append(secondBarNav);
             }else if(menuContainerStyle.indexOf("button-bar")!=-1){
                 writer.append("<div class=\"button-bar row\">");

             }
        }

        if (modelMenu.renderedMenuItemCount(context) > 0) {
            String admin_menu_ul = UtilProperties.getPropertyValue("coloradmin.properties", "admin.appbar.ul");
            if (UtilValidate.isEmpty(menuId)) {
                //使用Boots menu
                if(menuContainerStyle.indexOf("tab-bar")!=-1) {
                String barStyle = UtilProperties.getPropertyValue("coloradmin.properties", "admin.secondbar.ul");
   ...
   //Debug.logInfo("in renderMenuItem, menuItem:" + menuItem.getName() + " context:" + context ,"");
        ...
        if (link != null) {
            if (UtilValidate.isNotEmpty(menuId)||menuContainerStyle.indexOf("tab-bar")!=-1) {
                ...
                link.setStyle("btn btn-primary btn-sm");
            }

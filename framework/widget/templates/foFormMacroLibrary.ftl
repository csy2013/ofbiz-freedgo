...
<#escape x as x?xml>
    ...
    <#macro makeBlock style text>
    <fo:block<#if style?has_content> <@getFoStyle style/></#if>><#if text?exists>${text}</#if></fo:block></#macro>
    ...
    <fo:table border="solid black"><#list columnStyles as columnStyle>
        <fo:table-column<#if columnStyle?has_content> <@getFoStyle columnStyle/></#if>/></#list></#macro>
        <#macro renderFormatListWrapperClose formName></fo:table-body></fo:table></#macro>

    <#macro renderFormatHeaderRowOpen style>
    <fo:table-header>
    <fo:table-row></#macro>
    <#macro renderFormatHeaderRowClose></fo:table-row></fo:table-header>
    <fo:table-body>
    ...
    <fo:table-cell <#if positionSpan?has_content && positionSpan gt 1 >number-columns-spanned="${positionSpan}"</#if><@getFoStyle "listtitlestyle"/>>
    <fo:block></#macro>
    ...
    <fo:table-cell
        <#if positionSpan?has_content && positionSpan gt 1 >number-columns-spanned="${positionSpan}"</#if><#if style?has_content><@getFoStyle style/><#else><@getFoStyle "tabletext"/></#if>></#macro>
    ...
    <fo:table-cell font-weight="bold" text-align="right" padding="3pt">
    <fo:block></#macro>
    ...
    <fo:table-cell text-align="left" padding="2pt" padding-left="5pt" <#if positionSpan?has_content && positionSpan gt 1 >number-columns-spanned="${positionSpan}"</#if>></#macro>
    ...
    <#macro makeHyperlinkString linkStyle hiddenFormName event action imgSrc title alternate linkUrl targetWindow description confirmation imgStyle><@makeBlock linkStyle description /></#macro>
    <#macro renderTooltip tooltip tooltipStyle></#macro>
    <#macro renderAsterisks requiredField requiredStyle></#macro>

    <#macro renderInputGroupFormOpen linkUrl formType targetWindow containerId containerStyle autocomplete name viewIndexField viewSizeField viewIndex viewSize useRowSubmit></#macro>
    <#macro renderInputGroupFieldRowTitleCellOpen  style> </#macro>
    <#macro renderInputGroupFieldRowTitleCellClose></#macro>
    <#macro renderInputGroupWrapperOpen formName style> </#macro>
    <#macro rendeInputGroupWrapperEnd formName> </#macro>
    <#macro  renderInputGroupFieldRowOpen> </#macro>
    <#macro  renderInputGroupFieldRowClose> </#macro>
    <#macro  renderInputGroupBtnRowOpen> </#macro>
    <#macro renderInputGroupBtnRowTitleCellOpen  style> </#macro>
    <#macro renderInputGroupBtnRowTitleCellClose  style> </#macro>
    <#macro renderLookupField name formName fieldFormName className="" alert="false" value="" size="" maxlength="" id="" event="" action="" readonly=false autocomplete="" descriptionFieldName="" targetParameterIter="" imgSrc="" ajaxUrl="" ajaxEnabled=javaScriptEnabled presentation="layer" width="" height="" position="" fadeBackground="true" clearText="" showDescription="" initiallyCollapsed="" lastViewName="main" required="" tooltip=""></#macro>
    ...
    <#macro renderInputGroupFormClose focusFieldName formName containerId hasRequiredField></#macro>

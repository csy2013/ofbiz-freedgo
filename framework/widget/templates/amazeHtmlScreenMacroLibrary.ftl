...
<#macro renderLabel text id style>
    ...
    <div class="am-cf am-padding-xs am-text-lg">
        ...
        <#else>
            <span${idText} class="am-text-primary am-text-default">${text}</span>
        </#if>
    </div>
    </#if>
</#macro>
...
<#--<#if isFirst><div class="am-g"></#if><div class="am-u-sm-2 <#if isEnd>am-u-end</#if>">-->
    ...
    <div class="am-padding-ms am-cf"><a <#if id?has_content>id="${id}"</#if> class="am-btn am-btn-primary am-round am-btn-sm" <#if name?has_content>name="${name}"</#if>
        ...
        <#if imgStr?has_content>${imgStr}</#if><#if text?has_content>${text}</#if></a></div>
    <#else>
...
<#macro renderImage src id style wid hgt border alt urlString>
    <#if src?has_content>
    <img <#if id?has_content>id="${id}"</#if><#if style?has_content> class="${style}"</#if><#if wid?has_content> width="${wid}"</#if><#if hgt?has_content> height="${hgt}"</#if><#if border?has_content>
         border="${border}"</#if>
...
<div class="am-panel am-panel-default" <#if id?has_content> id="${id}"</#if>><#rt/>
    <#if showMore>
        <#if title?has_content>
            <div class="am-panel-hd am-cf am-text-lg"  <#if collapsed> data-am-collapse="{target: '#${collapsibleAreaId}'}" </#if>>
    ...
    ${menuString}
     ...
     -->
        <#if menuString?has_content>
            <div class="am-panel-bd am-collapse am-in">${menuString}</div>
        </#if>
    </#if>
...
<#macro renderPortalPageBegin originalPortalPageId portalPageId confMode="false" addColumnLabel="Add column" addColumnHint="Add a new column to this portal">
    <#if confMode == "true">
    <a class="am-btn am-btn-primary am-round am-btn-sm" href="javascript:document.addColumn_${portalPageId}.submit()" title="${addColumnHint}">${addColumnLabel}</a> <b>PortalPageId: ${portalPageId}</b>
...
<div class="am-u-lg-${avg} portal-column<#if confMode == "true">-config</#if> connectedSortable" style="vertical-align: top; " id="portalColumn_${columnSeqId}">
    <#if confMode == "true">
        <div class="am-btn-toolbar">
            <div class="am-btn-group">
                    <a class="am-btn am-btn-primary am-btn-sm" href="javascript:document.delColumn_${columnKey}.submit()" title="${delColumnHint}">${delColumnLabel}</a>
                    <a class="am-btn am-btn-primary am-btn-sm" role="button" href="javascript:document.addPortlet_${columnKey}.submit()" title="${addPortletHint}">${addPortletLabel}</a>
                    <a class="am-btn am-btn-primary am-btn-sm" href="javascript:document.setColumnSize_${columnKey}.submit()" title="${setColumnSizeHint}">${colWidthLabel}: ${width}</a>
            </div>
        </div>
        <br class="clear"/>
    </#if>

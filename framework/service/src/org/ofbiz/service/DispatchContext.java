...
public class DispatchContext implements Serializable {
    ...
    public static UtilCache<String, Map<String, ModelService>> modelServiceMapByComponent = UtilCache.createUtilCache("service.ModelServiceMapByComponent", 0, 0, false);
    public static Set<String> modelServiceComponentNames = FastSet.newInstance();
                ...
                if (serviceMap == null) {
                    ...
                    // get all of the component resource model stuff, ie specified in each ofbiz-component.xml file
                     for (ComponentConfig.ServiceResourceInfo componentResourceInfo: ComponentConfig.getAllServiceResourceInfos("model")) {
                         modelServiceComponentNames.add(componentResourceInfo.componentConfig.getComponentName());
                    ...
                    for (Map<String, ModelService> servicesMap: ExecutionPool.getAllFutures(futures)) {
                        if (servicesMap != null) {
                            ...
                            Iterator iter = servicesMap.keySet().iterator();
                            if (iter.hasNext()) {
                                String key = (String) iter.next();
                                String name =  servicesMap.get(key).componentName;

                                if(modelServiceMapByComponent.get(name)!=null){
                                    modelServiceMapByComponent.get(name).putAll(servicesMap);
                                }else{
                                    modelServiceMapByComponent.put(name,servicesMap);
                                }
                            }
                        }
                    }
    ...
    private Map<String, ModelService> getComponentServiceMap(String componentName) {
        Map<String, ModelService> serviceMap = modelServiceMapByComponent.get(componentName);
        if (serviceMap == null) {
            synchronized (this) {
                serviceMap = modelServiceMapByDispatcher.get(componentName);
                if (serviceMap == null) {
                    serviceMap = FastMap.newInstance();
                    ...
                    // get all of the component resource model stuff, ie specified in each ofbiz-component.xml file
                    for (ComponentConfig.ServiceResourceInfo componentResourceInfo: ComponentConfig.getAllServiceResourceInfos("model")) {
                        if(componentResourceInfo.componentConfig.getComponentName().equals(componentName)){
                            futures.add(ExecutionPool.GLOBAL_EXECUTOR.submit(createServiceReaderCallable(componentResourceInfo.createResourceHandler())));
                        }
                    ...
                    for (Map<String, ModelService> servicesMap: ExecutionPool.getAllFutures(futures)) {
                        if (servicesMap != null) {
                            ModelService modelService =  (ModelService)((List)serviceMap.values()).get(0);
                            modelServiceMapByComponent.put(modelService.componentName,serviceMap);
                            serviceMap.putAll(servicesMap);
                        }
                    }
                }
    ...
    public Set<String> getServiceNamesByComponentName(String name) {
        Set<String> serviceNames = new TreeSet<String>();

        Map<String, ModelService> globalServices = modelServiceMapByComponent.get(name);
        Map<String, ModelService> localServices = modelServiceMapByDispatcher.get(name);
        if (globalServices != null) {
            serviceNames.addAll(globalServices.keySet());
        }

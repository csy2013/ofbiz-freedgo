...
public class ModelService extends AbstractMap<String, Object> implements Serializable {
    ...
    public String toString() {
        ...
        buf.append(componentName).append("::");
    ...
    */
    ...
    public void validate(Map<String, Object> context, String mode, Locale locale) throws ServiceValidationException {
        ...
        try {
//            validate remove tenantId by changsy 2015.6.8 problem?
            if(requiredTest.containsKey("tenantId")){ requiredTest.remove("tenantId"); requiredInfo.remove("tenantId");}
            if(requiredTest.containsKey("webSiteId")) { requiredTest.remove("webSiteId");requiredInfo.remove("webSiteId");}
            if(requiredTest.containsKey("productStoreId")){  requiredTest.remove("productStoreId");requiredInfo.remove("productStoreId");}
            if(optionalTest.containsKey("tenantId")) { optionalTest.remove("tenantId");requiredInfo.remove("tenantId");}
            if(optionalTest.containsKey("webSiteId")) { optionalTest.remove("webSiteId");requiredInfo.remove("webSiteId");}
            if(optionalTest.containsKey("productStoreId")) { optionalTest.remove("productStoreId");requiredInfo.remove("productStoreId");}
            validate(requiredInfo, requiredTest, true, this, mode, locale);
            validate(optionalInfo, optionalTest, false, this, mode, locale);
            //            validate remove tenantId by changsy 2015.6.8
    ...
    public Map<String, Object> makeValid(Map<String, ? extends Object> source, String mode, boolean includeInternal, List<Object> errorMessages, TimeZone timeZone, Locale locale) {
        ...
        if (locale == null) {
            ...
            if (source.containsKey("locale")) {
                if(source.get("locale") instanceof  String){
                    locale = new Locale((String)source.get("locale") );
                }else {
            ...
            if (param.mode.equals("INOUT") || param.mode.equals(mode)) {
                ...
                // other attributes
                    ...
                    if (source.containsKey(key)) {
                        if (!param.internal || includeInternal) {

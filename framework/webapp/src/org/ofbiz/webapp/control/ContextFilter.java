...
public class ContextFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if ("Y".equals(useMultitenant)) {
            ...
            String tenantId = UtilHttp.getTenantByURL((HttpServletRequest)request);
            try {
                // if tenant was specified, replace delegator with the new per-tenant delegator and set tenantId to session attribute
                Delegator delegator = getDelegator(config.getServletContext());
                List<GenericValue> tenants = delegator.findList("Tenant", EntityCondition.makeCondition("tenantId", tenantId), null, UtilMisc.toList("-createdStamp"), null, true);
                if (UtilValidate.isNotEmpty(tenants)) {
                    ...
                    //默认的dispath,delegator
                    // make that tenant active, setup a new delegator and a new dispatcher
                    String tenantDelegatorName = "default";
                    httpRequest.getSession().setAttribute("delegatorName", "default");

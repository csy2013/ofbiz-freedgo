...
public class ServiceMultiEventHandler implements EventHandler {
    ...
    public String invoke(Event event, RequestMap requestMap, HttpServletRequest request, HttpServletResponse response) throws EventHandlerException {
        ...
        boolean useRowSubmit = request.getParameter("_useRowSubmit") != null && "Y".equalsIgnoreCase(request.getParameter("_useRowSubmit"));

        // check if we are to also look in a global scope (no delimiter)
        boolean checkGlobalScope = request.getParameter("_checkGlobalScope") == null || !"N".equalsIgnoreCase(request.getParameter("_checkGlobalScope"));
        ...
        try {
            ...
            for (int i = 0; i < rowCount; i++) {
                ...
                if (UtilValidate.isNotEmpty(request.getAttribute(UtilHttp.ROW_SUBMIT_PREFIX + i))) {
                    rowSelected = request.getAttribute(UtilHttp.ROW_SUBMIT_PREFIX + i) != null && "Y".equalsIgnoreCase((String) request.getAttribute(UtilHttp.ROW_SUBMIT_PREFIX + i));
                } else {
                    rowSelected = request.getParameter(UtilHttp.ROW_SUBMIT_PREFIX + i) != null && "Y".equalsIgnoreCase(request.getParameter(UtilHttp.ROW_SUBMIT_PREFIX + i));
                }
                ...
                if (result == null) {
                    ...
                    if (!UtilValidate.isEmpty(result.get(ModelService.SUCCESS_MESSAGE_LIST))) {
                        List<String> newSuccessMessages = UtilGenerics.checkList(result.get(ModelService.SUCCESS_MESSAGE_LIST));

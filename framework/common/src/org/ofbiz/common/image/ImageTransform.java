...
public class ImageTransform {
     ...
     * @return                  New scaled buffered image
     */
    public static Map<String, Object> scaleImage(BufferedImage bufImg, double imgHeight, double imgWidth, Map<String, Map<String, String>> dimensionMap, String sizeType, Locale locale,String extension,String newFilePath,String oldFilePath) {
        /* VARIABLES */
        BufferedImage bufNewImg;
        double defaultHeight, defaultWidth, scaleFactor;
        Map<String, Object> result = FastMap.newInstance();
        ...
        String scaleType = UtilProperties.getPropertyValue("catalog", "image.scale.type");
        if (scaleType.equals("imageMagick")||scaleType.equals("graphicsmagick")) {
            try {
                boolean isGraphicsMagick = false;
                if(scaleType.equals("graphicsmagick")){
                    isGraphicsMagick = true;
                }
                ImageMagickTransform.zoomImage(oldFilePath,newFilePath,new Double(imgWidth * scaleFactor).intValue(),new Double(imgHeight * scaleFactor).intValue(),isGraphicsMagick);
            } catch (InterruptedException e) {
                String errMsg = UtilProperties.getMessage(resource, "Magick zoomImage.InterruptedException", locale) + e.toString();
                Debug.logError(errMsg, module);
                result.put("errorMessage", errMsg);
                return result;
            } catch (IOException e) {
                ...
                String errMsg = UtilProperties.getMessage(resource, "Magic zoomImage.IM4JavaException", locale) + e.toString();
                Debug.logError(errMsg, module);
                result.put("errorMessage", errMsg);
                return result;
            }
            ...
            // write new image
            try {
                ImageIO.write(bufNewImg, extension, new File(newFilePath));

...
public class CommonWorkers {
     ...
     * Returns a list of regional geo associations.
     */
    public static List<Map> getAssociatedStateMap(Delegator delegator, String country, String listOrderBy) {
        if (UtilValidate.isEmpty(country)) {
            // Load the system default country
            country = EntityUtilProperties.getPropertyValue("general.properties", "country.geo.id.default", delegator);
        }
        ...
        List<Map> retGeo = FastList.newInstance();
        if(!geoList.isEmpty()){
            for (int i = 0; i < geoList.size(); i++) {
                GenericValue value = geoList.get(i);
                Map<String,Object> geoMap = FastMap.newInstance();
                geoMap.put("name",value.get("geoName"));
                geoMap.put("id",value.get("geoId"));
                retGeo.add(geoMap);
            }
        }

...
public class GeoWorker {
    ...
    public static GenericValue findListGeoPoint(Delegator delegator, String entityName) {
        List<GenericValue> gptList = null;
        try {
            gptList = delegator.findByAnd(entityName, null, UtilMisc.toList("-fromDate"));
            } catch (GenericEntityException e) {
                Debug.logError(e, "Error while finding List GeoPoint  " + e.toString(), module);
            }
    ...
    public static String getGeoNameById(String geoId, Delegator delegator) {
        GenericValue geo = null;
        try {
            geo = delegator.findByPrimaryKeyCache("Geo", UtilMisc.toMap("geoId", geoId));
        } catch (GenericEntityException e) {
        ...
        return geo==null?"":geo.getString("geoName");
    }
    
    public static String getGeoIdByName(String geoName, Delegator delegator) {
        GenericValue geo = null;
        try {
            geo = delegator.findByPrimaryKeyCache("Geo", UtilMisc.toMap("geoName", geoName));
        } catch (GenericEntityException e) {
            Debug.logError(e, "Unable to look up Geo from geoId : " + geoName, module);
        }
        
        return geo==null?"":geo.getString("geoId");
    }
    
    public static String getGeoIdByName1(String geoName, Delegator delegator) {
        GenericValue geo = null;
        try {
            geo = EntityUtil.getFirst(delegator.findByAndCache("Geo", UtilMisc.toMap("geoName", geoName)));
        } catch (GenericEntityException e) {
            Debug.logError(e, "Unable to look up Geo from geoId : " + geoName, module);
        }
        if( geo==null){
            ...
            String code = geo.getString("geoId");

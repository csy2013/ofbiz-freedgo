...
public class ComponentConfig {
    ...
    protected static Map<String, List<WebappInfo>> serverExistsWebApps = FastMap.newInstance();

    public static ComponentConfig getComponentConfig(String globalName,String useWebapps) throws ComponentException {
        // TODO: we need to look up the rootLocation from the container config, or this will blow up
        return getComponentConfig(globalName, null,useWebapps);
    }

    public static ComponentConfig getComponentConfig(String globalName) throws ComponentException {
        // TODO: we need to look up the rootLocation from the container config, or this will blow up
        return getComponentConfig(globalName, null,"true");
    }

    public static ComponentConfig getComponentConfig(String globalName, String rootLocation,String useWebapps) throws ComponentException {
        ...
        if (componentConfig == null) {
            if (rootLocation != null) {
                synchronized (ComponentConfig.class) {
                    ...
                    if (componentConfig == null) {
                        componentConfig = new ComponentConfig(globalName, rootLocation,useWebapps);
    ...
    public static List<WebappInfo> getExistsAppBarWebInfos(String serverName) {
        return ComponentConfig.getExistsAppBarWebInfos(serverName, null, null);
    }

    public static List<WebappInfo> getExistsAppBarWebInfos(String serverName, String menuName) {
        return ComponentConfig.getExistsAppBarWebInfos(serverName, null, menuName);
    }

    public static List<WebappInfo> getExistsAppBarWebInfos(String serverName, Comparator<? super String> comp, String menuName) {
        List<WebappInfo> webInfos = serverExistsWebApps.get(serverName + menuName);
        if (webInfos == null) {
            synchronized (ComponentConfig.class) {
                if (webInfos == null) {
                    Map<String, WebappInfo> tm = null;
                    ...
                    for (ComponentConfig cc: getAllComponents()) {
                        for (WebappInfo wInfo: cc.getExistsWebappInfos()) {
                            String key = UtilValidate.isNotEmpty(wInfo.position)? wInfo.position: wInfo.title;
                            if (serverName.equals(wInfo.server) && wInfo.appBarDisplay) {
                                if (UtilValidate.isNotEmpty(menuName)) {
                                    if (menuName.equals(wInfo.menuName)) {
                    ...
                    List<WebappInfo> webInfoList = FastList.newInstance();
                    webInfoList.addAll(tm.values());
                    serverExistsWebApps.put(serverName + menuName, webInfoList);
                    return webInfoList;
                }
            }
        }
    ...
    protected List<WebappInfo> existsWebappInfos = FastList.newInstance();
    protected ComponentConfig() {}

    protected ComponentConfig(String globalName, String rootLocation,String useWebapps) throws ComponentException {
        ...
        for (Element curElement : UtilXml.childElementList(ofbizComponentElement, "webapp")) {
            WebappInfo webappInfo = new WebappInfo(this, curElement);
            this.existsWebappInfos.add(webappInfo);
        }
    ...
    public List<WebappInfo> getExistsWebappInfos() {
        return this.existsWebappInfos;
    }

    public void setExistsWebappInfos(List<WebappInfo> existsWebappInfos) {
        this.existsWebappInfos = existsWebappInfos;
    }
